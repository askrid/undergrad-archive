...
def _emit_tri2(
    verts: list[float], norms: list[float], cols: tuple[int, ...],
    a: Point3, b: Point3, c: Point3,
    na: Point3, nb: Point3, nc: Point3,
    ca: Color, cb: Color, cc: Color,
) -> tuple[int, ...]:
    """Emit triangle double-sided: front (a,b,c) + back (a,c,b) with negated normals."""
    verts.extend((*a, *b, *c, *a, *c, *b))
    norms.extend((*na, *nb, *nc, *(-x for x in na), *(-x for x in nc), *(-x for x in nb)))
    return cols + ca + cb + cc + ca + cc + cb


def _emit_tri2_flat(
    verts: list[float], norms: list[float], cols: tuple[int, ...],
    a: Point3, b: Point3, c: Point3, col: Color,
) -> tuple[int, ...]:
...
def _emit_tri2_smooth(
    verts: list[float], norms: list[float], cols: tuple[int, ...],
    a: Point3, b: Point3, c: Point3,
    na: Point3, nb: Point3, nc: Point3, col: Color,
) -> tuple[int, ...]:
...
class Cube(Primitive):
    ...
    def __init__(self, size: float | Point3 = 1.0, color: Color = (200, 200, 200, 255)) -> None:
    ...
    def __init__(
        ...
        for i in range(stacks):
            ...
            for j in range(slices):
                ...
                self.colors = _emit_tri2_smooth(self.vertices, self.normals, self.colors,
                    v_tl, v_bl, v_br, n_tl, n_bl, n_br, C)
                self.colors = _emit_tri2_smooth(self.vertices, self.normals, self.colors,
                    v_br, v_tr, v_tl, n_br, n_tr, n_tl, C)
            ...
            if not theta_full:
                for t_cap in (t0, t1):
                    for i in range(stacks):
                        phi_hi = p1 - (p1 - p0) * (i / stacks)
                        phi_lo = p1 - (p1 - p0) * ((i + 1) / stacks)
                        self.colors = _emit_tri2_flat(self.vertices, self.normals, self.colors,
                            (0.0, 0.0, 0.0), P(phi_lo, t_cap), P(phi_hi, t_cap), C)
...
class Torus(Primitive):
    ...
    def __init__(
        self, major_radius: float = 1.0, minor_radius: float = 0.3,
        major_segments: int = 40, minor_segments: int = 20,
        major_range: tuple[float, float] = (0, 360),
        minor_range: tuple[float, float] = (0, 360),
        color: Color = (120, 180, 120, 255), caps: bool = True,
    ) -> None:
        ...
        for i in range(major_segments):
            ...
            for j in range(minor_segments):
                ...
                self.colors = _emit_tri2_smooth(self.vertices, self.normals, self.colors,
                    p00, p10, p11, n00, n10, n11, C)
                self.colors = _emit_tri2_smooth(self.vertices, self.normals, self.colors,
                    p00, p11, p01, n00, n11, n01, C)
        ...
        if caps:
            ...
            if not major_full:
                for u_cap in (u0, u1):
                    ...
                    for j in range(minor_segments):
                        ...
                        v_b = v0 + (v1 - v0) * (j + 1) / minor_segments
                        self.colors = _emit_tri2_flat(self.vertices, self.normals, self.colors,
                            center, P(u_cap, v_a), P(u_cap, v_b), C)
            ...
            if not minor_full:
                for i in range(major_segments):
                    ...
                    u_b = u0 + (u1 - u0) * (i + 1) / major_segments
                    self.colors = _emit_tri2_flat(self.vertices, self.normals, self.colors,
                        P(u_a, v0), P(u_b, v0), P(u_b, v1), C)
                    self.colors = _emit_tri2_flat(self.vertices, self.normals, self.colors,
                        P(u_a, v0), P(u_b, v1), P(u_a, v1), C)
...
class Frustum(Primitive):
    ...
    def __init__(
        self, bottom_radius: float = 1.0, top_radius: float = 0.5,
        height: float = 1.0, slices: int = 30,
        theta_range: tuple[float, float] = (0, 360),
        inner_bottom_radius: float = 0.0, inner_top_radius: float = 0.0,
        color: Color = (120, 140, 200, 255), caps: bool = True,
    ) -> None:
        Rb, Rt, Rbi, Rti, H = bottom_radius, top_radius, inner_bottom_radius, inner_top_radius, height
        t0, t1 = math.radians(theta_range[0]), math.radians(theta_range[1])
        C = color
        ...
        def N_outer(theta: float) -> Point3:
            ...
            return (H * math.cos(theta) / len_outer, (Rb - Rt) / len_outer, -H * math.sin(theta) / len_outer)

        def N_inner(theta: float) -> Point3:
            if len_inner < 1e-12:
                return (-math.cos(theta), 0.0, math.sin(theta))
            return (-H * math.cos(theta) / len_inner, (Rti - Rbi) / len_inner, H * math.sin(theta) / len_inner)
        ...
        def _lateral(P_fn, N_fn):
            for j in range(slices):
                ...
                self.colors = _emit_tri2_smooth(self.vertices, self.normals, self.colors,
                    p00, p10, p11, na, nb, nb, C)
                self.colors = _emit_tri2_smooth(self.vertices, self.normals, self.colors,
                    p00, p11, p01, na, nb, na, C)
        ...
        if caps:
            ...
            def _ring(h: float):
                for j in range(slices):
                    ...
                    self.colors = _emit_tri2_flat(self.vertices, self.normals, self.colors, ia, oa, ob, C)
                    self.colors = _emit_tri2_flat(self.vertices, self.normals, self.colors, ia, ob, ib, C)
            ...
            if not theta_full:
                for th_cap in (t0, t1):
                    ...
                    self.colors = _emit_tri2_flat(self.vertices, self.normals, self.colors, i_bot, o_bot, o_top, C)
                    self.colors = _emit_tri2_flat(self.vertices, self.normals, self.colors, i_bot, o_top, i_top, C)
...
class Axes(Primitive):
    ...
    def __init__(self, length: float = 5, tick: float = 1, tick_size: float = 0.08) -> None:
        self.vertices: list[float] = []
        self.normals: list[float] = []
        self.colors: tuple[int, ...] = ()
        self.indices: list[int] = []
        ...
        seg((0, 0, 0), (L, 0, 0), POS_X);  seg((0, 0, 0), (-L, 0, 0), NEG_X)
        seg((0, 0, 0), (0, L, 0), POS_Y);  seg((0, 0, 0), (0, -L, 0), NEG_Y)
        seg((0, 0, 0), (0, 0, L), POS_Z);  seg((0, 0, 0), (0, 0, -L), NEG_Z)

        # Arrowheads at positive tips
        ah, aw = tick_size * 3.0, tick_size * 1.5
        for tip, back_ax, col in [
            ((L, 0, 0), 0, POS_X), ((0, L, 0), 1, POS_Y), ((0, 0, L), 2, POS_Z)
        ]:
            for axis in range(3):
                if axis == back_ax:
                    continue
        ...
        while k * tick <= length + 1e-6:
            d = k * tick
            ts = tick_size
            seg((d, -ts, 0), (d, ts, 0), TICK_POS);  seg((d, 0, -ts), (d, 0, ts), TICK_POS)
            seg((-d, -ts, 0), (-d, ts, 0), TICK_NEG); seg((-d, 0, -ts), (-d, 0, ts), TICK_NEG)
            seg((-ts, d, 0), (ts, d, 0), TICK_POS);   seg((0, d, -ts), (0, d, ts), TICK_POS)
            seg((-ts, -d, 0), (ts, -d, 0), TICK_NEG); seg((0, -d, -ts), (0, -d, ts), TICK_NEG)
            seg((-ts, 0, d), (ts, 0, d), TICK_POS);   seg((0, -ts, d), (0, ts, d), TICK_POS)
            seg((-ts, 0, -d), (ts, 0, -d), TICK_NEG); seg((0, -ts, -d), (0, ts, -d), TICK_NEG)
            k += 1

...
int ip_black_list(struct sr_ip_hdr *iph)
{
	...
	/**************** fill in code here *****************/
	struct in_addr target, m, blocked;

	inet_aton(ip_blacklist, &target);
	inet_aton(mask, &m);

	if ((iph->ip_src & m.s_addr) == (target.s_addr & m.s_addr)) {
		blocked.s_addr = iph->ip_src;
		printf("[IP blocked] %s\n", inet_ntoa(blocked));
		return 1;
	}
	if ((iph->ip_dst & m.s_addr) == (target.s_addr & m.s_addr)) {
		blocked.s_addr = iph->ip_dst;
		printf("[IP blocked] %s\n", inet_ntoa(blocked));
		return 1;
	}
	return 0;
    ...
    */
	...
	if (e_hdr0->ether_type == htons(ethertype_ip))
	{
		...
		else
		{
			...
			if (rtentry != NULL)
			{
				...
				{
					/**************** fill in code here *****************/
					/* build ICMP time exceeded reply */
				...
				/* TTL not expired */
					...
					/**************** fill in code here *****************/
					/* decrement TTL and forward to next hop */
					i_hdr0->ip_ttl--;
					i_hdr0->ip_sum = 0;
					i_hdr0->ip_sum = cksum(i_hdr0, sizeof(struct sr_ip_hdr));
			...
			{
				/**************** fill in code here *****************/
				/* build ICMP destination net unreachable */
				new_len = sizeof(struct sr_ethernet_hdr) + sizeof(struct sr_ip_hdr) + sizeof(struct sr_icmp_t3_hdr);
				new_pck = (uint8_t *)calloc(1, new_len);
	...
	else if (e_hdr0->ether_type == htons(ethertype_arp))
	{
		...
		if (a_hdr0->ar_tip == ifc->ip)
		{
			...
			{
				/**************** fill in code here *****************/
				/* learn MAC, flush queued packets */
				...
				if (arpreq != NULL)
				{
					for (en_pck = arpreq->packets; en_pck != NULL; en_pck = en_pck->next)
					{
						...
						ifc = sr_get_interface(sr, en_pck->iface);

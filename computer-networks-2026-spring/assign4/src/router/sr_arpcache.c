...
void sr_arpcache_handle_arpreq(struct sr_instance *sr, struct sr_arpreq *req)
{
    ...
    */
    ...
    if (difftime(curtime, req->sent) > 1.0)
    {
        ...
        {
            /**************** fill in code here *****************/
            /* send ICMP host unreachable to each queued sender */
            for (pck = req->packets; pck != NULL; pck = pck->next)
            {
                ...
                struct sr_ethernet_hdr *e_new = (struct sr_ethernet_hdr *)out;
                i_hdr = (struct sr_ip_hdr *)(out + sizeof(struct sr_ethernet_hdr));
                ict3_hdr = (struct sr_icmp_t3_hdr *)(out + sizeof(struct sr_ethernet_hdr) + sizeof(struct sr_ip_hdr));
                ...
                e_new->ether_type = htons(ethertype_ip);
                memcpy(e_new->ether_shost, ifc->addr, ETHER_ADDR_LEN);

                entry = sr_arpcache_lookup(cache, i_hdr->ip_dst);
                if (entry != NULL)
                {
                    memcpy(e_new->ether_dhost, entry->mac, ETHER_ADDR_LEN);
                    free(entry);
                    sr_send_packet(sr, out, len, rtentry->interface);
                }
                else
                {
                    struct sr_arpreq *q = sr_arpcache_queuereq(cache, i_hdr->ip_dst, out, len, rtentry->interface);
                    sr_arpcache_handle_arpreq(sr, q);
                }
                free(out);
            }
            /****************************************************/
        ...
        {
            /**************** fill in code here *****************/
            /* (re)send broadcast ARP request, bump counters */

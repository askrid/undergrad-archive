  ...
  *)
  ...
  let malloc_with_gc s m e c k =
    if List.length m < mem_limit then
      ...
      let rec mark_sval sv =
        match sv with
        | V v -> mark_val v
        | P (_, _, env) -> List.iter mark_eval env
        | M evp -> mark_eval evp
      and mark_eval (_, ev) =
        match ev with
        | Loc l -> mark l
        | Proc(_, _, env) -> List.iter mark_eval env
      and mark_kval (_, env) = List.iter mark_eval env
      and mark l =
      ...
      and mark_val v =
        match v with
        | L l -> mark l
        | R r -> List.iter mark (snd (List.split r))
        | _ -> ()
      in

      let _ = List.iter mark_sval s in
      let _ = List.iter mark_eval e in
      let _ = List.iter mark_kval k in

      let new_m = List.filter (fun (l, _) -> List.mem_assoc (fst l) !reachable_locs) m in
